...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            ...
            name=op.f("ck_cases_case_ownership_scope_valid"),
        ),
        sa.CheckConstraint(
            "status IN ('draft', 'published', 'archived')", name=op.f("ck_cases_case_status_valid")
        ),
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["source_reference_id"], ["source_references.id"], ondelete="SET NULL"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index(op.f("ix_evidence_packets_case_id"), "evidence_packets", ["case_id"])
    op.create_index(
        op.f("ix_evidence_packets_source_reference_id"), "evidence_packets", ["source_reference_id"]
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["evidence_packet_id"], ["evidence_packets.id"], ondelete="SET NULL"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index(op.f("ix_decision_points_case_step_id"), "decision_points", ["case_step_id"])
    op.create_index(
        op.f("ix_decision_points_evidence_packet_id"), "decision_points", ["evidence_packet_id"]
    )
