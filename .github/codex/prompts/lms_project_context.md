...
- **Autofix lane.** Lint/format autofix should never touch `docs/product/*.md` content. If `docs/architecture/decision-records/` exists in the current branch, treat those ADRs the same way. These are research/design artifacts where wording is load-bearing.
