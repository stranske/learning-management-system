...
REMEDIATION_TRIGGER_TYPES: tuple[str, ...] = (
    "failed-prerequisite",
...
class RemediationTrigger(Base):
    """Rule that maps evidence or attempt signals to remediation scheduling."""

    __tablename__ = "remediation_triggers"
    __table_args__ = (
        CheckConstraint(
            f"trigger_type IN ({_sql_values(REMEDIATION_TRIGGER_TYPES)})",
            name="remediation_trigger_type_valid",
        ),
        CheckConstraint(
            f"ownership_scope IN ({_sql_values(OWNERSHIP_SCOPES)})",
            name="remediation_trigger_ownership_scope_valid",
        ),
    )
    ...
    trigger_rules: Mapped[dict[str, Any]] = mapped_column(JSON, nullable=False)
