...
- PR routing: #217 is open, ready-for-review, non-draft, closes #197, registry branch, labels `agent:codex`, `agents:keepalive`, `autofix`, `priority:normal`. Relay events emitted `issue_created` for #197 and `pr_opened` for #217 with `next_action=wait_for_keepalive`.
