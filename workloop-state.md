...
- PR: [#225](https://github.com/stranske/learning-management-system/pull/225) — ready-for-review, non-draft, `Closes #201`, branch `codex/issue-201-inspect-scheduler-panel`, labels `agent:codex`, `agents:keepalive`, `autofix`, `agent:retry`, `priority:low`, `audit-2026-05-30`. Fired `pr_opened active.source_repo=stranske/learning-management-system active.source_issue=201 active.source_pr=225 active.next_action=wait_for_keepalive`.
- Post-open repair/cap: `opener-repair-infra-stalls.py` dispatched Gate Followups for #225 after initial `needs-dispatch-evidence`. Fresh cap-health at 21:03Z reports raw cap below 5, #225 `draining` (`keepalive summary reports an agent currently running`), #5367 `draining`, and only #5353 remaining non-drainable (`runner-failed` scoped owner/product blocker). Direct PR evidence includes keepalive summary/work-log comments and runner-dispatch comment for codex at head `e05dc10`.
- Next action: keepalive owns #225 CI/review follow-up; opener can continue to next candidate on the next lane tick.
...
- Source issue/PR: [#197](https://github.com/stranske/learning-management-system/issues/197) / [#217](https://github.com/stranske/learning-management-system/pull/217), branch `codex/issue-197-drift-scan-freetext-range`, commit `644db4c`.
- Selection context: mandatory cap/liveness discovery ran. Raw opener cap was 4/5 before selection and remained below cap after repair preflight; PAEM #1847 and Trend #5353 stayed scoped/non-drainable, LMS #215 and #216 had fresh draining evidence, and LMS #195/#196 were already linked to open PRs. #197 was the next unlinked normal-priority implementation issue.
- Implementation: markdown-file references now validate `passage_range` before hashing, including the explicit-content path, and reject unsupported free-text ranges instead of silently hashing explicit passage content at create time and whole-file content at scan time. `_select_passage` now raises the same unsupported-range error if reached directly.
- Tests: added `tests/sources/test_drift_scan.py::test_freetext_passage_range_not_false_stale` covering the ambiguous free-text range path.
- Validation: `uv run pytest tests/sources/test_drift_scan.py::test_freetext_passage_range_not_false_stale -q --no-cov` -> 1 passed; `uv run pytest tests/sources/ -q --no-cov` -> 16 passed; `uv run ruff check src/lms/sources/repository.py tests/sources/test_drift_scan.py` -> passed. Deliberate-break gate: restored the old silent whole-file fallback and removed the validation; the named test failed with `DID NOT RAISE`; restored the guard and reran green.
- PR routing: #217 is open, ready-for-review, non-draft, closes #197, registry branch, labels `agent:codex`, `agents:keepalive`, `autofix`, `agent:retry`, `priority:normal`. Relay events emitted `issue_created` for #197 and `pr_opened` for #217 with `next_action=wait_for_keepalive`. After a stale exhausted-autofix comment/`needs-human` label appeared for superseded head `644db4c`, opener removed the stale blocker label and dispatched Gate Followups run `26712237520` with `force_retry=true` for current head `50f1a29`.
- Workspace hygiene: worktree lives under `/Users/teacher/.codex/automations/pd-workloop-resume/worktrees/lms-issue-197-drift-scan`, not the Code root. The worktree has an untracked `uv.lock` generated by `uv run`; it was intentionally not committed.
- Next action: keepalive owns PR #217 CI/review; closer owns post-merge verification.
